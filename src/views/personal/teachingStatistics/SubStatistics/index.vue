    ...
    <div class="search-area">
      <el-form :inline="true" :model="queryParams" class="form-inline" label-width="85px">
        <el-form-item label="日期范围：">
          <el-radio :label="1" v-if="platTermList.length" v-model="queryParams.radio">按学期</el-radio>
          ...
          <el-table
            ...
            <el-table-column>
              <template slot="header">
                <span v-if="queryParams.headerPath.length > 0">
                  <a @click="goToLevel(0)">{{theadName}}</a>
                  <span :key="item.id" v-for="(item, index) in queryParams.headerPath">
                    -<a @click="goToLevel(item)" v-if="index < queryParams.headerPath.length-1">{{item.name}}</a>
                    <span class="color-disable" v-else>{{item.name}}</span>
                  </span>
                </span>
                <span v-else>{{theadName}}</span>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.dataType === 'classroom'">{{scope.row.name}}</span>
                <a @click="goToNext(scope.row)" type="link" v-else>{{scope.row.name}}</a>
              </template>
            </el-table-column>
            <el-table-column
              label="计划课时数"
...
export default {
  ...
  methods: {
    ...
    goToLevel (item) {
      this.loading = true
      if (item === 0) {
        this.queryParams.headerPath = []
        this.queryParams.areaId = this.baseAreaId
        this.queryParams.isDirectSchool = 0
      } else {
        const start = this.queryParams.headerPath.indexOf(item) + 1
        const num = this.queryParams.headerPath.length - start
        this.queryParams.headerPath.splice(start, num)
        // console.log('this.queryParams.headerPath', this.queryParams.headerPath)
        if (item.dataType === 'area') {
          this.queryParams.areaId = item.id
          this.queryParams.isDirectSchool = 0
        } else if (item.dataType === 'school') {
          this.queryParams.schoolId = item.id
          this.queryParams.isDirectSchool = 0
        } else if (item.dataType === 'direct') {
          this.queryParams.areaId = item.id
          this.queryParams.isDirectSchool = 1
        }
      }
      this.initTableList()
    ...
    /**
     * @description: 点击区域列去查下一级
     * @param arguments 描述
     * @returns {}
     */
    goToNext (row) {
      this.loading = true
      this.queryParams.headerPath.push(row)
      console.log('添加path', this.queryParams.headerPath)
      // 强制刷新el-table组件 为了解决table slot = header情况下不自动刷新问题
      if (row.dataType === 'area') {
        this.queryParams.areaId = row.id
        this.queryParams.isDirectSchool = 0
      } else if (row.dataType === 'school') {
        this.queryParams.schoolId = row.id
        this.queryParams.isDirectSchool = 0
      } else if (row.dataType === 'direct') {
        this.queryParams.areaId = row.id
        this.queryParams.isDirectSchool = 1
      }
      this.initTableList()
