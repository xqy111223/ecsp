      ...
      <el-table-column :class-name="getClassName(item)" :key="item.prop" :label="item.label" :width="getIndex(index)" v-for="(item, index) in columns">
        <template slot-scope="scope">
          ...
          <div class="cl-section" v-else-if="item.prop==='number'">
            {{scope.row.number}}
            <p class="ft12 c6" v-if="showTime">{{ scope.row.startTime }} - {{ scope.row.endTime }}</p><i class="el-icon-custom-sign"></i>
          </div>
          ...
          <div @click="triggerEvent(scope.row)" class="box-radius" v-else-if="item.prop && scope.row[item.prop] && scope.row[item.prop].length">
            <el-popover placement="top-start" trigger="click" width="260">
              <!-- 课程信息浮层 内容 -->
              <class-information :classData="scope.row[item.prop]" :noMore="noMore"></class-information>
              <div slot="reference">
...
export default {
  ...
  props: {
    ...
    showTime: { // 课表中节次对应时间是否展示
      type: Boolean
    }
  },
  data () {
    return {
      noMore: false, // 课表浮层 是否 有更多信息通道
      columns: [], // 课表初始化渲染 - 循环列
      tableData: [], // 课表数据
      spanArray: [], // 合并单元格
